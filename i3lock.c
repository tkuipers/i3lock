...
int main(int argc, char *argv[]) {
	...
	rgba_colors[3] = 1; // set alpha channel to 0 (transparent)
	char *username;
	...
	while ((o = getopt_long(argc, argv, optstring, longopts, &optind)) != -1) {
		...
		default:
			...
			" [-i image.png] [-t] [-e] [-I] [-f] [--inverse-wheel]"
			);
		}
	}
 ...
 * run i3lock as super-user). */
	...
	if(is_inverse){
		if (image_path) {
			...
			char output[1035];
			char cmd[1035];
			char *foo = "/usr/bin/convert ";
			char *bar = " -resize 1x1! out.txt && cat out.txt | grep -Po 'srgb\\(\\K[^\\)]*' && rm out.txt";
			strcpy(cmd, foo);
			...
			errx(EXIT_FAILURE, "Cannot use --inverse-wheel unless you have specified -i\n");
		}
	}
	else{
		RED=255;
